...
let AuthController = class AuthController {
    ...
    async register(registerDto) {
        try {
            return await (0, rxjs_1.firstValueFrom)(this.authServiceClient.send("register", registerDto));
        }
        catch (e) {
    ...
    async login(loginDto) {
        try {
            return await (0, rxjs_1.firstValueFrom)(this.authServiceClient.send("login", loginDto));
        }
        catch (e) {
            const { status, message } = this.normalizeError(e);
            throw new common_1.HttpException(message, status);

...
function detectStatus(err: any): number {
  ...
  if (msg.includes('unauthorized')) return HttpStatus.UNAUTHORIZED
  if (msg.includes('forbidden')) return HttpStatus.FORBIDDEN
  if (msg.includes('already') || msg.includes('conflict') || msg.includes('not on transfer list')) return HttpStatus.CONFLICT
...
export async function wrapAsync<T>(fn: () => Promise<T>, context: string): Promise<T> {
  try {
    return await fn()
  } catch (err) {
